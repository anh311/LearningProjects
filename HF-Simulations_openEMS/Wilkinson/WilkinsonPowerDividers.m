...
CSX = AddBox(CSX,"Copper", 1,[-50, 10, 1.5],[-12, 13, 1.5]); %Line
CSX = AddBox(CSX,"Copper", 1,[1.5-15,3.4,1.5], [3-15,10, 1.5]);
CSX = AddBox(CSX,"Copper", 1,[1.5-15,13,1.5], [3-15,19.6, 1.5]);
CSX = AddBox(CSX,"Copper", 1,[1.5-15, 3.4, 1.5],[31.5-15,1.9 , 1.5]);
CSX = AddBox(CSX,"Copper", 1,[1.5-15, 19.6, 1.5],[31.5-15,21.1 , 1.5]);
CSX = AddBox(CSX,"Copper", 1,[31.5-15, 3.4+0.75, 1.5],[50,1.9-0.75 , 1.5]);
CSX = AddBox(CSX,"Copper", 1,[31.5-15, 19.6-0.75, 1.5],[50,21.1+0.75 , 1.5]);

CSX = AddLumpedElement( CSX, 'Resistor', 1, 'Caps', 1, 'R', 100 );
CSX = AddBox( CSX, 'Resistor', 1, [31.5-15 3.4+0.75 1.5], [31.5-12 19.6-0.75 1.5] );
%define mesh
...
%define Ports
[CSX,port{1}] = AddMSLPort( CSX, 9, 1, 'Copper', [-50 10 1.5], [-20 13 0], 0, [0 0 -1], 'ExcitePort',true,'FeedShift', 10*resolution, "Feed_R",50);
[CSX,port{2}] = AddMSLPort( CSX, 9, 2, 'Copper', [50 3.4+0.75 1.5], [30 1.9-0.75 0], 0, [0 0 -1],'ExcitePort', false, "Feed_R",50);
[CSX,port{3}] = AddMSLPort( CSX, 9, 3, 'Copper', [50 19.6-0.75 1.5], [30 21.1+0.75 0], 0, [0 0 -1],'ExcitePort', false,"Feed_R",50);
...
%Save Data
WriteOpenEMS("temp/WilkinsonPowerDividers.xml",FDTD, CSX);

%model Display
CSXGeomPlot("temp/WilkinsonPowerDividers.xml");

%run simulation
RunOpenEMS( "temp", "WilkinsonPowerDividers.xml" );
